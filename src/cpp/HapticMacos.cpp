...
#ifdef HACTIC_DEBUG
  HAPT_DEBUG_LOG_INFO("Elapsed: " + std::to_string(elapsed.count()));
#endif

  if (elapsed < minDelay) {
#ifdef HACTIC_DEBUG
    HAPT_DEBUG_LOG_INFO("Can't trigger: Minimum delay is  " + std::to_string(minDelay.count()) + "ms but only " + std::to_string(elapsed.count()) + "ms have passed. ");
#endif
  } else {
    haptic_trigger(type);
  }
...
{
  // Default delay = 100ms
  minDelay = std::chrono::milliseconds(1);
